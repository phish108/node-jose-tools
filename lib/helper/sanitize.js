...
function getParent(url, levels) {
    return new Array(levels).fill().reduce((p) => path.dirname(p), url); // eslint-disable-line no-undef
}
...
export async function sanitize(toolname) {
    ...
    try {
        const dirpath = os.type() === "Windows_NT" ? basepath : basepath.replace("file://", "");
        tool = await fs.stat(path.join(dirpath,`${toolname}.js`));
    }
    catch (err) {
