...
function getParent(location, levels) {
    return new Array(levels)
        ...
        .reduce(
            (p) => path.dirname(p),
            url.fileURLToPath(new URL(location)) // eslint-disable-line no-undef
        );
}
...
export async function sanitize(toolname) {
    ...
    try {
        tool = await fs.stat(path.resolve(basepath,`${toolname}.js`));
    }
    catch (err) {
...
export async function execute(toolname, parameters) {
    const { default: tool, options, requireArgs } = await import(path.resolve(basepath,`${toolname}.js`));
