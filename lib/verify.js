...
module.exports = async function (args) {
    ...
    const rawkeystore = await loadFile(jwksFn);
    const keystore = await jose.JWK.asKeyStore(rawkeystore);
    ...
    if (payload.iat > epoc) {
        throw new Error("token time is too early");
    }
    ...
    if (payload.aud !== aud) {
        throw new Error("token is for the wrong audience");
    }
